...
for plane in planes.keys():
    ...
    if os.path.isdir(inDirBase+'/'+plane+'/'+flightDates[0]):
        ...
        for min in range(1,flightTimeMin):
            ...
            fout.write('        <description><![CDATA[{a} {b}<br>Lat: {c:0.4f}<br>Lon: {d:0.4f}<br>Alt: {e:0.1f} m<br>]]></description>\n'.format(a=lastDate_str,b=lastHrMin_str,c=df_sub.iloc[-1].lat,d=df_sub.iloc[-1].lon,e=df_sub.iloc[-1].gps_msl_alt))
            ...
            for ind in range(0,len(df_sub.index)-1):
                ...
                if currMinutes%5 == 0 and currMinutes != lastMinUsed:
                    ...
                    fout.write('        <description><![CDATA[{a}<br>Lat: {b:0.4f}<br>Lon: {c:0.4f}<br>Alt: {d:0.1f} m<br>]]></description>\n'.format(a=currTime_str,b=df_sub.iloc[ind].lat,c=df_sub.iloc[ind].lon,d=df_sub.iloc[ind].gps_msl_alt))
                    fout.write('        <TimeStamp>\n')
