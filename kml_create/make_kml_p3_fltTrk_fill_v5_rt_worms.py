...
outDirBase = '/home/disk/funnel/impacts-website/archive_ncar/gis/aircraft_worms'
missingValue = -999
kmlPrefix = 'gis.'
...
for plane in planes.keys():
    ...
    if os.path.isdir(inDirBase+'/'+plane+'/'+flightDates[0]):
        ...
        for min in range(1,2):
            ...
            if ((endTime - df.iloc[0].time).seconds / 60) < trackLenMinutes:
                startTime_track = df_sub.iloc[0].time
            else:
                startTime_track = df_sub.iloc[-1].time - timedelta(minutes=trackLenMinutes)
