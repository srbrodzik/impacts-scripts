...
if numargs != 4:
    print('Usage: {} [takeoffDate] [trackLenMinutes(15/30/60)] [imageIntervalMinutes(mult of 5)]'.format(sys.argv[0]))
    exit()
else:
    takeoffDate = sys.argv[1]
    trackLenMinutes = sys.argv[2]
    imageIntervalMinutes = sys.argv[3]
...
catalogBaseUrl = 'http://catalog.eol.ucar.edu/impacts_2023/aircraft/nasa_er2'
...
for plane in planes.keys():
    ...
    if os.path.isdir(inDirBase+'/'+plane+'/'+flightDates[0]):
        ...
        # Create kml files for current minute
            ...
            for ind in range(0,len(df_sub.index)-1):
                if df_sub.iloc[ind].time >=startTime_track:
                    ...
                    if currMinutes%int(imageIntervalMinutes) == 0 and currMinutes != lastMinUsed:
                        ...
                        currHour_str = currTime_dt.strftime("%H")

                        fout.write('      <Placemark>\n')

                        image = 'aircraft.NASA_ER2.'+currDateTime_str+'.radar_all_refl.png'
                        imageFullPath = imageDirBase+'/'+currDate_str+'/'+image
                        if os.path.isfile(imageFullPath):
                            fout.write('        <name>{a}: {b} {c}</name>\n'.format(a=planes[plane],b=currDate_str,c=currTime_str))
                            fout.write('        <description><![CDATA[Location - Lat: {a:0.4f}   Lon: {b:0.4f}<br>\n'.format(a=df_sub.iloc[ind].lat,b=df_sub.iloc[ind].lon))
                            fout.write('        <a width=700 href="'+catalogBaseUrl+'/'+currDate_str+'/'+currHour_str+'/'+image+'" target="blank">\n')
                            fout.write('        <img width=700  src="'+catalogBaseUrl+'/'+currDate_str+'/'+currHour_str+'/'+image+'">]]></description>\n')
