...
for plane in planes.keys():
    ...
    if os.path.isdir(inDirBase+'/'+plane+'/'+flightDates[0]):
        ...
        #endTime = df.iloc[0].time + timedelta(minutes=flightTimeMin)
        endTime = df.iloc[-1].time
        if ((endTime - startTime).seconds / 60) < 60:
            startTime_lastHour = df.iloc[0].time
        else:
            startTime_lastHour = endTime - timedelta(minutes=60)
        ...
        for ind in range(0,len(df.index)):
            if df.iloc[ind].time < startTime_lastHour:
        ...
        for ind in range(0,len(df.index)-1):
            ...
            if currMinutes%5 == 0 and currMinutes != lastMinUsed:
                ...
                fout.write('        <description><![CDATA[Time: {a} UTC<br>Lat: {b:0.4f}<br>Lon: {c:0.4f}<br>Alt: {d:0.1f} m<br>Temp: {e:0.2f} C<br>]]></description>\n'.format(a=currTime_str,b=df.iloc[ind].lat,c=df.iloc[ind].lon,d=df.iloc[ind].gps_msl_alt,e=df.iloc[ind].amb_temp))
                fout.write('        <TimeStamp>\n')
