...
# v2 adds temp and aircraft name and removes redundant time to/from clickable info
# v3 color codes track so last hour is in red and older track is in yellow
# v4 color codes 5-minute dots so last hour is in red and older is yellow
...
if numargs != 3:
    print('Usage: {} [takeoffDate] [trackLenMinutes(15/30/60)]'.format(sys.argv[0]))
    exit()
else:
    takeoffDate = sys.argv[1]
    trackLenMinutes = sys.argv[2]
    print('takeoffDate = {}'.format(takeoffDate))
    print('trackLenMinutes = {}'.format(trackLenMinutes))

# User inputs
test = False
inDirBase = '/home/disk/bob/impacts/raw/aircraft'
outDirBase = '/home/disk/funnel/impacts-website/archive_ncar/gis/aircraft_'+trackLenMinutes+'min'
missingValue = -999
kmlPrefix = 'gis.'
kmlSuffix = 'flight_track_'+trackLenMinutes+'min'
...
# Get current time and fill flightDates list
now = datetime.utcnow()
now_str = now.strftime("%Y%m%d%H%M")
now_date = now.strftime("%Y%m%d")
if takeoffDate < now_date:
    flightDates = [takeoffDate,now_date]
else:
...
for plane in planes.keys():
    ...
    if os.path.isdir(inDirBase+'/'+plane+'/'+flightDates[0]):
        ...
        iwgFile = '/tmp/'+plane+'.'+now_str+'.'+trackLenMinutes+'.csv'
        ...
        """
        # Only keep last 60 minues in df
        df = df.set_index('time')
        df = df.loc[df.index > (df.index[-1]-timedelta(hours=1))]
        ...
        # Create kml file for current minute
        #for min in range(1,flightTimeMin):
        for min in range(1,2):
            endTime = df.iloc[-1].time
            print('{} {}'.format('endTime = ',endTime))
            df_sub = df[df['time'] <= endTime]
            if ((endTime - df.iloc[0].time).seconds / 60) < int(trackLenMinutes):
                startTime_track = df_sub.iloc[0].time
            else:
                startTime_track = df_sub.iloc[-1].time - timedelta(minutes=int(trackLenMinutes))
            ...
            fout.write('        <scale>0.5</scale>\n')
            fout.write('        <Icon>\n')
            fout.write('          <href>http://catalog.eol.ucar.edu/kmlicons/yellow_dot.png</href>\n')
            fout.write('        </Icon>\n')
            fout.write('      </IconStyle>\n')
            fout.write('    </Style>\n')
            ...
            # Output lon/lat/alt triads more than trackLenMinutes old to kml file
            for ind in range(0,len(df_sub.index)):
                if df_sub.iloc[ind].time < startTime_track:
            ...
            # Output lon/lat/alt triads from last trackLenMinutes to kml file
            for ind in range(0,len(df_sub.index)):
                if df_sub.iloc[ind].time >= startTime_track:
                    fout.write('            {a:0.4f},{b:0.4f},{c:0.1f}\n'.format(a=df_sub.iloc[ind].lon,b=df_sub.iloc[ind].lat,c=df_sub.iloc[ind].gps_msl_alt))
            ...
            fout.write('        <description><![CDATA[Time: {a} UTC<br>Lat: {b:0.4f}<br>Lon: {c:0.4f}<br>Alt: {d:0.1f} m/{e:0.3f} kft<br>Temp: {f:0.2f} C<br>]]></description>\n'.format(a=lastHrMin_str,b=df_sub.iloc[-1].lat,c=df_sub.iloc[-1].lon,d=df_sub.iloc[-1].gps_msl_alt,e=M2KFT*(df_sub.iloc[-1].gps_msl_alt),f=df_sub.iloc[-1].amb_temp))
            ...
            # Output all previous positions in last trackLenMinutes if minutes are multiples of 5
            lastMinUsed = -1
            for ind in range(0,len(df_sub.index)-1):
                if df_sub.iloc[ind].time >=startTime_track:
                    ...
                    if currMinutes%5 == 0 and currMinutes != lastMinUsed:
                        ...
                        fout.write('        <description><![CDATA[Time: {a} UTC<br>Lat: {b:0.4f}<br>Lon: {c:0.4f}<br>Alt: {d:0.1f} m/{e:0.3f} kft<br>Temp: {f:0.2f} C<br>]]></description>\n'.format(a=currTime_str,b=df_sub.iloc[ind].lat,c=df_sub.iloc[ind].lon,d=df_sub.iloc[ind].gps_msl_alt,e=M2KFT*(df_sub.iloc[ind].gps_msl_alt),f=df_sub.iloc[ind].amb_temp))
                        fout.write('        <TimeStamp>\n')
                        fout.write('          <when>{a}Z</when>\n'.format(a=currTime_str))
                        fout.write('        </TimeStamp>\n')
                        if currTime_dt < startTime_track:
