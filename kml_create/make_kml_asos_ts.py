...
oneHrAgo = datetime.utcnow() - timedelta(hours=1)
oneHrAgoDate = oneHrAgo.strftime('%Y%m%d')
dateList = [oneHrAgoDate]
if nowDate != oneHrAgoDate:
  dateList.append(nowDate)

for date in dateList:
  if date in os.listdir(indir):
    hourList = []
    ...
    for fileHour in uniqueHours:
      ...
      if not os.path.exists(kmlName):
        print('{} {}'.format('Creating:',kmlFile))
        fout = open(kmlName,'w')
