    ...
    if os.path.isdir(inDirBase+'/'+plane+'/'+flightDates[0]):
        ...
        for min in range(1,flightTimeMin):
            ...
            for ind in range(0,len(df_sub.index)):
                #fout.write(f'{df_sub.iloc[ind].lon},{df_sub.iloc[ind].lat},{df_sub.iloc[ind].gps_msl_alt}\n')  # too many digits
            ...
            fout.write('      <Placemark>\n')
            fout.write('        <name>{a}: {b} {c}UTC</name>\n'.format(a=planes[plane],b=lastDate_str,c=lastHrMin_str))
            fout.write('        <description><![CDATA[Lat: {a:0.4f}<br>Lon: {b:0.4f}<br>Alt: {c:0.1f} m<br><br>Temp: {d:0.2f} C<br>]]></description>\n'.format(a=df_sub.iloc[-1].lat,b=df_sub.iloc[-1].lon,c=df_sub.iloc[-1].gps_msl_alt,d=df_sub.iloc[-1].amb_temp))
            ...
            for ind in range(0,len(df_sub.index)-1):
                ...
                if currMinutes%5 == 0 and currMinutes != lastMinUsed:
                    ...
                    fout.write('        <description><![CDATA[Lat: {a:0.4f}<br>Lon: {b:0.4f}<br>Alt: {c:0.1f} m<br><br>Temp: {d:0.2f} C<br>]]></description>\n'.format(a=df_sub.iloc[ind].lat,b=df_sub.iloc[ind].lon,c=df_sub.iloc[ind].gps_msl_alt,d=df_sub.iloc[ind].amb_temp))
                    fout.write('        <TimeStamp>\n')
