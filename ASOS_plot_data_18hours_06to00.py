...
Make 18 hour plots from 0600 one day to 0000 the next for a list of ASOS weather stations.
Data is read from csv files created by ASOS_get_site_data.py

**File Saving Information**
18-hour plots, from 0600 of datelist day to 0000 of the following day
save to: '/home/disk/funnel/impacts/archive/ops/asos_18hr_06to00'
'''
import os 
import pandas as pd 
import urllib 
...
sitelist = {'KALB':'ALBANY INTL AP',
            'KART':'WATERTOWN AP',
            'KIAG':'NIAGARA FALLS INTL AP',
            'KITH':'ITHACA TOMPKINS REG AP',
            'KMPV':'MONTPELIER AP',
            'KRME':'ROME GRIFFISS AIRFIELD',
            'KROC':'ROCHESTER GTR INTL',
            'KRUT':'RUTLAND VERMONT REG AP',
            'KSCH':'SCHENECTADY CTY AP',
            'CYTR':'TRENTON ONT AP'}
datelist = ['20200207']

csv_dir = '/home/disk/funnel/impacts/data_archive/asos_csv'
plot_dir = '/home/disk/funnel/impacts/archive/ops/asos_18hr_06to00'
...
def load_station_data(date,site):
    ...
    if os.path.exists(path0_file):
        ...
        today_18hrs = today_all.loc[today_all.index >= today_start]
    else: 
    ...
    *saves plots to plot_dir listed near top of script*
    ...
    for key in indices.keys():
        if indices[key] == 1:
            legend.append(key)
    if len(legend) == 1:
        # CYTR crashes on this statement
    ...
    for ax in axes: 
        ...
        # print xtick marks at 06, 12 and 18 hours
        #ax.xaxis.set_minor_locator( HourLocator(np.linspace(6,18,3)) )
        # print hourly xtick marks from 06 to 23
        ax.xaxis.set_minor_locator( HourLocator(np.linspace(6,23,18)) )
