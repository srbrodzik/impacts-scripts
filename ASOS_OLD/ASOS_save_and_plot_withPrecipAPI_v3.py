...
def load_and_save_station_data(site):
    ...
    *Saves .csv files to csv_dir listed near top of script*
    ...
    if os.path.exists(path3_file) and os.path.exists(path2_file) and os.path.exists(path1_file):
        ...
        if os.path.exists(path0_file):
            ...
            if earliest_time_to_midnight > 60: #Gets data since midnight
                time_diff = now - datetime.datetime(now.year,now.month,now.day,0,0)
                time_length = str( int( np.round(time_diff.days + time_diff.seconds/60,0) + 24*MINS_IN_HOUR ) )
    ...
    try:
        # ***** NEED TO CHECK FOR ONLY EMPTY PRECIP DATA AND CREATE EMPTY DATAFRAME *****
    ...
    if os.path.exists(path3_file) and os.path.exists(path2_file) and os.path.exists(path1_file):
        ...
        # ***** HOW DO WE KNOW WHEN PRECIP DATA HAS UPDATED SO WE CAN GRAB IT A WRITE OVER CSV FILE?? *****
        # ***** DO WE EXTEND time_length BY A FEW HOURS FOR EACH REQUEST & GET RID OF DUPS?? *****
        # ***** IF SO WE NEED TO ROUTINELY OVERWRITE today AND yesterday CSV FILES *****
        ...
        yesterday_data = yesterday_data.sort_index()
...
def plot_station_data(site,sitetitle,df):
    ...
    *saves plots to plot_dir listed near top of script*
    ...
    #plot_path = '/home/disk/bob/impacts/bin'
