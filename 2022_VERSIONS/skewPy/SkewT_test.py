...
#        for tick, loc, label in self.get_major_ticks():
#        for tick, loc, label in self.get_major_ticks(),self.xtics():
#            if tick is None: continue
#            if transforms.interval_contains(interval, loc):
#                tick.set_label1(tick.label)
#                tick.set_label2(tick.label)
#            tick.update_position(tick.loc)
#            tick.draw(renderer)
...
# Copyright (c) 2013 Thomas Chubb
    ...
    def add_profile(self,bloc=0.5, pres_s = 1000.0, **kwargs):
        ...
        if 'alpha' in kwargs: balph=kwargs['alpha']
        else: balph = 1.
