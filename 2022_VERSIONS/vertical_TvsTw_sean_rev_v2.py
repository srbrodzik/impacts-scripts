...
It assumes output files names are of the form <phrase>.<phrase>.<datetime>.<site>.<extension>

Suggested mods (so that output file name doesn't matter): 
- add input for datetime
- add input for site
...
def make_vertical_T_plot(sounding_wb_data,vert_lim,filename,outfile):
    ...
    axL.set_ylim(0,int(vert_lim)) #Sets the vertical limits (THIS MAY HAVE TO BE CHANGED)
    ...
    axR.set_ylim(0,int(vert_lim)) #Maintaining the vertical limit to keep both axes equal
    axR.set_yticks(np.arange(0,int(vert_lim)+1)) #Y ticks corresponding to the left axis (may be changed if the left axis tick marks are specified)
    axR.set_yticklabels(int(np.round(i*KM2FEET)) for i in np.arange(0,int(vert_lim)+1)) #Converts km into ft and rounds to the nearest foot
    ...
    #Assumes filename is of form ops.text_sounding.YYYYMMDDhhmm.<site>.html
...
def main():
    ...
    if len(sys.argv) != 4:
        raise SystemExit("Useage: {} {} {}".format(sys.argv[0], "[infile]", "[outfile] [vert_lim]"))
    filename = sys.argv[1]
    outfile = sys.argv[2]
    vert_lim = sys.argv[3]
    ...
    print('vert_lim = '+vert_lim)

    #filename = "/home/disk/bob/impacts/bin/ops.text_sounding.201910041200.WAL.html"
    #filename = "/home/disk/funnel/olympex/archive/ops/text_sounding/20150825/ops.text_sounding.201508250000.SLE.html"
