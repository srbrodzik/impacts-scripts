...
Difference from v3:
Tried to separate mwr vars with different time dimensions in case I wanted to create 
separate netcdf files for each set of vars.  Was doing this in the hope that I could 
concatenate original json data so that I could get complete day's data in one file.
That didn't work out so no need for this code yet.
'''
            ...
            for file in fileList:
                ...
                with open(file,'r') as f:
                    ...
                    if type(data.get('mwr')) == dict:
                        ...
                        timeKeys = ['temperature_qc','vapor_density_qc','liquid_qc','relative_humidity_qc']
                        timeRangeKeys = ['liquid','dew_point','relative_humidity','vapor_density',
                                         'pressure_level','temperature']
                        timeIntKeys = ['cloud_base','integrated_vapor','integrated_liquid','integrated_qc']
                        timeSfcKeys = ['rain_flag']

                        # ****** Check to make sure dim coords are part of subdictionaries ******
                        timeData = {key:mwr['data_vars'][key] for key in timeKeys if key in mwr['data_vars'].keys()}
                        timeRangeData = {key:mwr['data_vars'][key] for key in timeRangeKeys if key in mwr['data_vars'].keys()}
                        timeIntData = {key:mwr['data_vars'][key] for key in timeIntKeys if key in mwr['data_vars'].keys()}
                        timeSfcData = {key:mwr['data_vars'][key] for key in timeSfcKeys if key in mwr['data_vars'].keys()}
                        ...
                        mwr_timeData = xr.Dataset.from_dict(timeData)
                        mwr_timeRangeData = xr.Dataset.from_dict(timeRangeData)
                        mwr_timeIntData = xr.Dataset.from_dict(timeIntData)
                        mwr_timeSfcData = xr.Dataset.from_dict(timeSfcData)
