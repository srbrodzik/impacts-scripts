...
def plot_station_data(plotDir, site, dt, time_start, time_end, site_data, logo_path):
    ...
    Returns:
    Saves plots to plotDir listed near top of MAIN CODE*
    '''
...
endTime_date_obj = datetime.strptime(endTime_date_string,'%Y%m%d')
date_list = []
for idate in range(0,4):
    date_list.append( (endTime_date_obj-timedelta(hours=24*idate)).strftime('%Y%m%d') )
date_list.sort()

# get lower case verion of site
