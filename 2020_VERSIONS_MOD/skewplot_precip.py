...
from skewPy_precip import SkewT
import os
import datetime
...
for fname in sounding_files:
    ...
    ##Added by Sean O'Neil (Nov 2019)
        ...
        out_fname = 'upperair.{stn}_{name}.{dt}.skewT.png'.format(stn=stn_id,name=stn_info_cwb[stn_id]['name'],dt=file_time.strftime(file_out_dt_fmt))
        figtitle = '{name} ({stn}) {dt} sounding ({lat:.3f} {lon:.3f})'.format(name=stn_info_cwb[stn_id]['name'],stn=stn_id,dt=file_time.strftime(title_dt_fmt),
                                                                               lat=stn_info_cwb[stn_id]['lat'], lon=stn_info_cwb[stn_id]['lon'])
