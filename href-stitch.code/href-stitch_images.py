...
Execution
----------
python3 href-stitch_images.py latest ne,mw qpf_001h_mean_ptype /home/disk/bob/impacts/model/href_03km_test/ 
...
# TEMPORARY - UW web server path
uw_server_root = '/home/disk/funnel/impacts-website/archive/model/href_03km_test'
...
#                         + prod1_fname.split('/')[-1][:-4])
                    outfile = (
                        model_dir + prod + '/combined/' + 'model.HREF_03km'
                        + '.' + datetime.strftime(run_cycle, '%Y%m%d%H%M')
                        + '.' + str(fhr).zfill(3) + '_' + prod + '_' + region)
...
# UNCOMMENT FOR FIELD OPS
# Open ftp connection to NCAR sever
#catalogFTP = FTP(ftpCatalogServer,ftpCatalogUser)
#catalogFTP.cwd(catalogDestDir)

# Upload combined images to Field Catalog and delete local raw and combined images
for run_cycle in run_cycle_list:
    model_dir = root_dir + datetime.strftime(run_cycle, '%Y%m%dT%H') + '/'
    # ----- TEMPORARY - for writing files to UW web server -----
    uw_server_dir = uw_server_root + '/' + datetime.strftime(run_cycle, '%Y%m%d')
    if not os.path.isdir(uw_server_dir):
        os.makedirs(uw_server_dir)
        shutil.copy(uw_server_root + '/index.php',
                    uw_server_dir)
    # ----------------------------------------------------------
    #for region in region_list:
    for prod in prod_list:
        raw_path = model_dir + prod + '/raw/'
        for file in os.listdir(raw_path):
            os.remove(os.path.join(raw_path,file))
        combined_path = model_dir + prod + '/combined/'
        for file in os.listdir(combined_path):
        #   UNCOMMENT FOR FIELD OPS
        #    ftpFile = open(os.path.join(combined,file),'rb')
        #    catalogFTP.storbinary('STOR '+file,ftpFile)
        #    ftpFile.close()
        #    os.remove(os.path.join(combined_path,file))
        # ----- TEMPORARY - write files to UW web server -----
            shutil.copy(os.path.join(combined_path,file),
                        os.path.join(uw_server_dir,file) )
        # ----------------------------------------------------
