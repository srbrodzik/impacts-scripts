...
#Global Variable
#Name or path of figure that will be segmented to find the nw corner for pixel conversion
image_name = 'Impacts_test_figure.png'
#directory of pickle files
pickle_jar = '/home/disk/p/broneil/Documents/Impacts/ASOS_map/pickle_jar/'
#Directory of plots stored on the website
html_plots_directory = 'http://www.atmos.washington.edu/~broneil/impacts/plots'
...
def write_box2txt(nw_corner,se_corner,station,name):
    ...
    '''
    with open('clickable_boxes.txt','a') as f:
        line = ('                <area shape=\"rect\" coords=\"%s,%s,%s,%s\" alt=\"%s\" href=\"%s/%s.html\" title=\"%s\">\n'
               % (nw_corner[0], nw_corner[1], se_corner[0], se_corner[1], station, html_plots_directory, station.lower(), name))
        f.write(line)
