...
"""
'''
ASOS_plot_data_1day_12to12.py
Make 1-day plots from noon to noon for a list of ASOS weather stations.
Data is read from csv files created by ASOS_get_data_KALX.py

**File Saving Information**
1-day plots, from noon of datelist day to noon of the following day, save to: '/home/disk/funnel/impacts/archive/ops/asos_1day_12to12'
'''
import os 
import pandas as pd 
import urllib 
...
plot_dir = '/home/disk/funnel/impacts/archive/ops/asos_1day_12to12'
...
def load_station_data(date,site):
    ...
    # define time limits
    today_noon = now.strftime('%Y-%m-%d')+' 12:00:00'
    tomorrow_noon = (now+timedelta(hours=24)).strftime('%Y-%m-%d')+' 12:00:00'
    
    # define data path and filename
    path0_dir = csv_dir+'/'+today_date
    path1_dir = csv_dir+'/'+tomorrow_date
    
    path0_file = path0_dir+'/ops.asos.'+today_date+'.'+lower_site+'.csv'
    path1_file = path1_dir+'/ops.asos.'+tomorrow_date+'.'+lower_site+'.csv'
    
    if os.path.exists(path0_file) and os.path.exists(path1_file):
        today_all = pd.read_csv(path0_file)
        today_all['time'] = pd.to_datetime(today_all['time'])
        today_all = today_all.set_index('time')
        today_12hrs = today_all.loc[today_all.index >= today_noon]

        tomorrow_all = pd.read_csv(path1_file)
        tomorrow_all['time'] = pd.to_datetime(tomorrow_all['time'])
        tomorrow_all = tomorrow_all.set_index('time')
        tomorrow_12hrs = tomorrow_all.loc[tomorrow_all.index <= tomorrow_noon]
    else: 
        print('some data is missing -- go to next plot')
    
    # concatenate three DataFrame objects into one
    frames = [today_12hrs, tomorrow_12hrs]
    df = pd.concat(frames)
