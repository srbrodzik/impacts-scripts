...
date_1_day_ago = datetime.datetime.now() - timedelta(hours=hoursBack) + UTC_OFFSET_TIMEDELTA
...
numTimes = hoursBack/24 + 1
if numTimes == 2:
    dateStrList = [startDateStr,nowDateStr]
else:
    dateStrList = []
    for itime in range(0,numTimes):
        pythonDate = date_1_day_ago + timedelta(hours=24*itime)
        dateStrList.append(pythonDate.strftime("%Y%m%d") )
