...
BNLbaseDir = baseDir+'/BNL'
RTbaseDir = baseDir+'/RadarTruck'
products = []
BNLproducts = ['mrrpro2white','parsivel','pluvio','vceilo15k']
RTproducts = []
...
for product in BNLproducts:

    if product == 'mrrpro2white':
        print('Renaming mrrpro2 files...')
        path = BNLbaseDir+'/'+product
        prefix_out = prefix_base_out+'_'+product[0:7]
        suffix_out = 'BNL'
        for dir in os.listdir(path):
            if dir.startswith('2023') and os.path.isdir(path+'/'+dir):
                path_new = path+'/'+dir
                for fname_in in os.listdir(path_new):
                    if fname_in.endswith('nc'):
                        print('fname_in = ',fname_in)
