...
precip_map_dir_base = '/home/disk/funnel/impacts-website/archive/ops/totals_qc'
    ...
    for i in filelist: # Loops through the files
        with open(Data_Path + i, 'r') as f:
            ...
            if field == 5:
                ...
                precip_intervals = df.iloc[:,field].dropna()
                daily_accum_precip = float(precip_intervals.sum())
                data = list(df.iloc[-1,:].values)
...
def plot_ASOS(ASOS,field,date):    
    ...
    precip_map_name = 'ops.totals.'+date+time+'.precip24.png'
